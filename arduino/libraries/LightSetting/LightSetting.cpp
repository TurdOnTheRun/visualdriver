...
byte LightSetting::init(unsigned long now, byte lightid)
{
        ...
        case BEZIERDIMM: {
          ...
          if(rising()){
            ...
            if (_bezierstep >= 100){
              ...
              _bz = bezier(_bezierstep);
              _newstate = (int) round(_fromstate - _bz * (_fromstate - _tostate));
              _state = lowByte(_newstate);
            }
          }
        } break;
...
{ 
  ...
  // The Green Lines
  _ya = lerp( 0 , _set2 , _i );
  _yb = lerp( _set2 , _set4 , _i );
  _yc = lerp( _set4 , 100 , _i );
